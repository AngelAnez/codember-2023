...
let separated = message.split(" ")
let originals = []
let quantity = []
separated.forEach((elemento) => {
  let word = elemento.toLowerCase()
  if (!originals.includes(word)){
    originals.push(elemento)
    quantity.push(0)
  }
})
let result = []
separated.forEach((elemento) => {
  ...
  let location = originals.indexOf(elemento.toLowerCase())
  quantity[location]++
  result[location] = elemento.toLowerCase() + quantity[location]
